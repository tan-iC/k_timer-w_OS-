...
void checker_function(void)
{
	...
	OBJECT(semaphore_4,SERIAL_RCV_SEM2);
	OBJECT(semaphore_3,SERIAL_SND_SEM1);
	OBJECT(semaphore_5,SERIAL_SND_SEM2);
	OBJECT(semaphore_1,SID1);
	...
	OBJECT(cyclic_1,ROT_CYC_HANDLER);
	...
	OBJECT(interrupt_0,12);
	OBJECT(interrupt_1,INHNO_SIO);
	OBJECT(interrupt_2,INHNO_SIO2);
